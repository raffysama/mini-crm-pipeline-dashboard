...
export function AppLayout({
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* Mobile sidebar */}
      <aside
        className={`fixed left-0 top-0 z-30 h-screen w-60 flex-col border-r border-slate-200 bg-white transition-transform md:hidden ${mobileOpen ? "flex translate-x-0" : "flex -translate-x-full"}`}
      >
        {/* Brand */}
        <div className="border-b border-slate-100 px-5 py-5">
          <div className="flex items-center gap-2.5">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 md:hidden">
        <div className="flex items-center gap-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-slate-900">
            <i className="ti ti-chart-dots text-xs text-white" />
          </div>
          <p className="text-sm font-semibold text-slate-900">Mini CRM</p>
        </div>
        <button onClick={() => setMobileOpen(!mobileOpen)}>
          <i
            className={`ti ${mobileOpen ? "ti-x" : "ti-menu-2"} text-xl text-slate-600`}
          />
        </button>
      </div>

      <main className="md:pl-60 pt-[53px] md:pt-0">{children}</main>
    </div>
  );
}
